...
//POLAROIDDDDDDDDDDDDDDDDDDD
const polaroidPhotos = [
  { image: "Images/us1.jpg", caption: "our first trip 💕" },
  { image: "Images/us2.jpg", caption: "birthday dinner 🎂" },
  { image: "Images/us3.jpg", caption: "our late night walk 🌙" },
];

let currentPolaroid = 0;
const polaroidPhoto = document.querySelector('.photo');
const polaroidCaption = document.querySelector('.caption');
const polaroidLeftArrow = document.querySelector('.polaroidarrowleft');
const polaroidRightArrow = document.querySelector('.polaroidarrowright');

function loadPolaroid(index) {
  const p = polaroidPhotos[index];
  polaroidPhoto.src = p.image;
  polaroidCaption.textContent = p.caption;
}

function slidePolaroid(direction) {
  const oldPolaroid = polaroidPictures.cloneNode(true);
  const newPolaroid = polaroidPictures.cloneNode(true);
  const parent = polaroidPictures.parentElement;

  const dir = direction === "right" ? 1 : -1;
  const newIndex = (currentPolaroid + dir + polaroidPhotos.length) % polaroidPhotos.length;
  const p = polaroidPhotos[newIndex];

  newPolaroid.querySelector('.photo').src = p.image;
  newPolaroid.querySelector('.caption').textContent = p.caption;

  newPolaroid.style.transform = `translateX(${dir * 100}%)`;
  parent.appendChild(newPolaroid);

  requestAnimationFrame(() => {
    oldPolaroid.style.transform = `translateX(${-dir * 100}%)`;
    oldPolaroid.style.opacity = "0";
    newPolaroid.style.transform = "translateX(0)";
  });

  setTimeout(() => {
    parent.removeChild(oldPolaroid);
  }, 600);

  currentPolaroid = newIndex;
